...
- **Сид-контент**: два ивента из дизайн-дока Lindenmoor (текст диалогов и
  опций взят дословно оттуда) внутри Folder «Lindenmoor — Events»:
  - **Passenger** — старушка и кот на голове гиганта, 3 опции, 3 разных
    исхода (Steady Hands / Cat Hat / штраф к ценам).
  - **Field Harvest** — «Завидный урожай» → «Мельница»: второй Event Actor
    (Мельница) заблокирован, пока не помочь фермерше с урожаем — по доку
    ивент "недоступен, если не убрали урожай". Это разряжено через ноду
    Unlock: ветка "Help harvest it" ведёт в Unlock, чей выходной пин
    подключён к триггер-пину Event Actor «Mill» (второй триггер туда же
    идёт от своей Location). Наглядная демонстрация того, как один ивент
    открывает следующий.
